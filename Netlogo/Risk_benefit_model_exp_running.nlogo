...
turtles-own [
  ...
  weight_avg_risk_benefit ; farmers only
...
to assign-weights
  ...
  ; All farmers care about the opinion of their consumers with medium weight
  set weight_avg_risk_benefit 0.5
...
;  setup-external-factors
  ...
  set frequency_talk_consumers frequency_talk_consumers + 1
...
;  setup-external-factors
  ...
  set frequency_talk_farmers frequency_talk_farmers + 1
...
;; If there is a low water demand, there is a higer perceived risk and lower benefit (the benefit is assumed)
  if is-farmer? self [
    ...
    ; Normalize between x in [a, b] (-1 and 1)
    ; ((b - a) * ((x - min x) / (max x - min x)) + a)
    ; does not work
    ;set risk risk + weight_avg_risk_benefit * ((1 - (-1)) * ((average_risk_consumers - 0) / (70 - 0)) + (-1))
    ;set benefit benefit + weight_avg_risk_benefit * ((1 - (-1)) * ((average_benefit_consumers - 0) / (70 - 0)) + (-1))
...
;  ]
  ...
  ask person1 [
   set new_risk ([risk] of person1 - ((([risk] of person1 - avg_conversation_risk) / F)) / 10)
   set new_benefit ([benefit] of person1 - ((([benefit] of person1 - avg_conversation_benefit) / F) / 10))
   setRandB new_risk new_benefit
   changecluster
   setxy risk benefit]

  ask person2 [
   set new_risk ([risk] of person2 - ((([risk] of person2 - avg_conversation_risk) / F)/ 10 ))
   set new_benefit ([benefit] of person2 - ((([benefit] of person2 - avg_conversation_benefit) / F) / 10))
   setRandB new_risk new_benefit
   changecluster
   setxy risk benefit]
end
...
;    ]
    ...
    ifelse item i ext_change_list = "decreasing"
      [ifelse item i ext_rate_list = "gradual"
        [set ext_factors_list replace-item i ext_factors_list max (list -1 (x - 0.0001))]
        [ifelse item i ext_rate_list = "medium"
          [set ext_factors_list replace-item i ext_factors_list max (list -1 (x - 0.0002))]
          [set ext_factors_list replace-item i ext_factors_list max (list -1 (x - random-float 0.0005))] ; temporary
        ]
    ]
      [if item i ext_change_list = "increasing"
        [ifelse item i ext_rate_list = "gradual"
          [set ext_factors_list replace-item i ext_factors_list min (list 1 (x + 0.0001))]
          [ifelse item i ext_rate_list = "medium"
           [set ext_factors_list replace-item i ext_factors_list min (list 1 (x + 0.0002))]
           [set ext_factors_list replace-item i ext_factors_list min (list 1 (x + random-float 0.0005))] ; temporary
          ]
        ]
      ]
