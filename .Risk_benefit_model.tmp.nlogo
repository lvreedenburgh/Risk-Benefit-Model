...
consumers-own [
  ...
  consumer_leader ; 0 if not leader, >1 if leader
...
farmers-own [
  ...
  network ; list of consumers in network
  network_size ; size of network
  farmer_leader ; 1 if not leader, >1 if leader
...
to setupconsumers
  ...
  create-consumers consumers_total [set color red set shape "person" set size 7 set network []]



  let n 0
  while [n < consumer_leaders][
        ask one-of consumers with [ consumer_leader < leader_influence] [
            set consumer_leader leader_influence set size 10.5
            set n n + 1
      ]]

  ask consumers [set clustered 0]

  ask n-of 4 consumers with [clustered = 0] [ set cluster "optimistic" set clustered 1]
  ask n-of 4 consumers with [clustered = 0] [ set cluster "neutral" set clustered 1]
  ask n-of 4 consumers with [clustered = 0] [ set cluster "alarmed" set clustered 1]
  ask n-of 4 consumers with [clustered = 0] [ set cluster "conflicted" set clustered 1]

  ask consumers with [cluster = "optimistic"] [set risk random-normal 24.3 7.1 set benefit random-normal 57.7 6.5]
  ask consumers with [cluster = "neutral"] [set risk random-normal 36.1 6.5 set benefit random-normal 39.1 7.0]
  ask consumers with [cluster = "alarmed"] [set risk random-normal 55.2 7.8 set benefit random-normal 26.5 8.8]
  ask consumers with [cluster = "conflicted"] [set risk random-normal 46.3 7.1 set benefit random-normal 48.8 6.8]

  ask consumers [setxy risk benefit]

  ask consumers [setupnetwork]
